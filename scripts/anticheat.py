...
LEGAL_ITEMS = dict({
                        ...
                        #equipment
                        (4,0):(1,25,26,27), #chest warrior (iron), mage (silk), ranger(linen), rogue(cotton)
                        (5,0):(1,25,26,27), #gloves warrior (iron), mage (silk), ranger(linen), rogue(cotton)
                        (6,0):(1,25,26,27), #boots warrior (iron), mage (silk), ranger(linen), rogue(cotton)
                        (7,0):(1,25,26,27), #shoulder warrior (iron), mage (silk), ranger(linen), rogue(cotton)
...
class AntiCheatConnection(ConnectionScript):
    def on_join(self):
        if not self.on_equipment_update() or not self.on_level_update() or not self.on_skill_update():
            return False
...
def calculate_power_level(level):
    return 0

def remove_cheater(connection, reason):
    log(LOG_MESSAGE.format(playername = connection.name, ip = connection.address.host, reason = reason))
    connection.send_chat(DISCONNECT_MESSAGE.format(name = CUWO_ANTICHEAT, reason = reason))
...
def has_illegal_items(entity_data):
    for slotindex in range(13):
        ...
        if is_item_illegal(entity_data.equipment[slotindex]):
            return True
        if is_equiped_illegal(entity_data.equipment[slotindex], entity_data, slotindex):
            return True
        
    return False

def is_item_illegal(item, entity_data = None, in_slotindex=-1):
    # Rarity is what makes the items truely broken and overpowered, this is loose from level, but a rarity of 255 will make even a lvl 1 weapon do 2 billion damage.
    if item.rarity > LEGAL_RARITY:
        log("item rarity: {rarity}".format(rarity = item.rarity), LOG_LEVEL_VERBOSE)
        return True
    
    if not LEGAL_ITEMS.has_key((item.type, item.sub_type)) or not item.material in LEGAL_ITEMS[(item.type, item.sub_type)]:
        log("invalid material or item:type={type} subtype={subtype} material={material}".format(type = item.type, subtype = item.sub_type, material = item.material), LOG_LEVEL_VERBOSE)
        return True
...
def is_equiped_illegal(item, entity_data, in_slotindex):
    ...
    if calculate_power_level(item.level) > calculate_power_level(entity_data.level):
        log("item level too high for character", LOG_LEVEL_VERBOSE)
        return True

    if not LEGAL_ITEMSLOTS.has_key(item.type) or not in_slotindex in LEGAL_ITEMSLOTS[item.type]:
        log("item in invalid slot:type={type} subtype={subtype} slot={slot}".format(type = item.type, subtype = item.sub_type, slot = in_slotindex), LOG_LEVEL_VERBOSE)
        pass #pass for now, list incomplete
...
def has_illegal_skills(entity_data):
    ...
    for item in entity_data.skills:
        if item < 0:
            log("negative amount of skill points in a skill", LOG_LEVEL_VERBOSE)
            return True
        
        totalskillpoints = totalskillpoints + item
    
    if totalskillpoints > (entity_data.level -1) * 2:
        log("spend more skills then are available", LOG_LEVEL_VERBOSE)
        return True
